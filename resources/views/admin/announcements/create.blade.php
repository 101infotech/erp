...
<div class="max-w-4xl">
    <div class="mb-6">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                ...
                <p class="text-slate-400 mt-1">Send important updates to your team</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.announcements.store') }}" method="POST"
        ...
        <div class="mb-6">
            ...
            <p class="text-slate-500 text-sm mt-1">Write a clear and concise message for your team</p>
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="priority" required
                ...
                <option value="normal" {{ old('priority', 'normal' )=='normal' ? 'selected' : '' }}>Normal - Standard
                    updates</option>
                <option value="high" {{ old('priority')=='high' ? 'selected' : '' }}>High - Urgent/Important</option>
            </select>
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="recipient_type" id="recipient_type" required
                class="w-full px-4 py-2 bg-slate-700 border border-slate-600 text-white rounded-lg focus:ring-2 focus:ring-lime-500 focus:border-lime-500">
                <option value="all" {{ old('recipient_type', 'all' )=='all' ? 'selected' : '' }}>All Staff Members
                </option>
                <option value="specific" {{ old('recipient_type')=='specific' ? 'selected' : '' }}>Specific Employees
                </option>
            </select>
        </div>

        <!-- Specific Recipients -->
        <div id="specific_recipients" class="mb-6 {{ old('recipient_type') == 'specific' ? '' : 'hidden' }}">
            <label class="block text-sm font-medium text-slate-300 mb-2">
                Select Recipients
            </label>
        ...
        <div class="mb-6 space-y-3">
            <label class="flex items-center">
                <input type="checkbox" name="send_email" value="1" {{ old('send_email', true) ? 'checked' : '' }}
