...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-400 mt-1">Manage system users and their access</p>
        </div>
                    ...
                    <tr class="hover:bg-slate-700/50 transition">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div
                                    class="flex-shrink-0 h-10 w-10 bg-lime-500/20 rounded-full flex items-center justify-center">
                                    <span class="text-lime-400 font-semibold text-sm">{{ substr($user->name, 0, 2)
                                        }}</span>
                                </div>
                                <div class="ml-4">
                                    ...
                                    <div class="mt-1.5">
                                        <span
                                            class="px-2 py-0.5 text-xs font-semibold rounded-full {{ $user->role === 'admin' ? 'bg-lime-500/20 text-lime-400' : 'bg-slate-700 text-slate-300' }}">
                                            {{ ucfirst($user->role) }}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">{{ $user->created_at->format('M
                            d, Y') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center justify-end gap-1.5">
                                <!-- View Button -->
                                <a href="{{ route('admin.users.show', $user) }}"
                                    class="inline-flex items-center justify-center w-8 h-8 bg-lime-500/10 text-lime-400 rounded-lg hover:bg-lime-500/20 transition"
                                    title="View User">
                                ...
                                <div class="relative inline-block">
                                    <button type="button"
                                        class="inline-flex items-center justify-center w-8 h-8 bg-yellow-500/10 text-yellow-400 rounded-lg hover:bg-yellow-500/20 transition"
                                        onclick="togglePasswordMenu('password-menu-{{ $user->id }}')"
                                    ...
                                    <div id="password-menu-{{ $user->id }}"
                                        class="hidden absolute right-0 mt-2 w-64 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden">
                                        <button type="button"
                                            onclick="openSetPasswordModal({{ $user->id }}, '{{ $user->name }}', '{{ $user->email }}')"
                                            class="w-full text-left px-4 py-3 text-sm text-slate-300 hover:bg-slate-700 hover:text-white transition border-b border-slate-700">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="flex-1">
                                                    <div class="font-medium text-white">Set New Password</div>
                                        ...
                                        <form action="{{ route('admin.users.send-reset-link', $user) }}" method="POST">
                                            ...
                                            <button type="submit"
                                                ...
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div class="flex-1">
                                                        <div class="font-medium text-white">Send Reset Link</div>
                                        ...
                                        <form action="{{ route('admin.users.reset-password', $user) }}" method="POST">
                                            ...
                                            <button type="submit"
                                                ...
                                                class="w-full text-left px-4 py-3 text-sm text-slate-300 hover:bg-slate-700 hover:text-white transition">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        class="flex-shrink-0 w-8 h-8 bg-yellow-500/10 rounded-lg flex items-center justify-center">
                                                        <svg class="w-4 h-4 text-yellow-400" fill="none"
                                                            stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                stroke-width="2"
                                                    ...
                                                    <div class="flex-1">
                                                        <div class="font-medium text-white">Generate & Email</div>
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-4">
            <div class="flex items-start justify-between mb-3">
                <div class="flex items-center space-x-3">
                    <div class="flex-shrink-0 h-12 w-12 bg-lime-500/20 rounded-full flex items-center justify-center">
                        <span class="text-lime-400 font-semibold">{{ substr($user->name, 0, 2) }}</span>
                    </div>
                    <div>
                ...
                <span
                    class="px-2 py-1 text-xs font-semibold rounded-full {{ $user->role === 'admin' ? 'bg-lime-500/20 text-lime-400' : 'bg-slate-700 text-slate-300' }}">
                    {{ ucfirst($user->role) }}
                </span>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-3 text-sm">
                <div>
                    <span class="text-slate-400">Jibble:</span>
                    @if($jibbleEmployee)
                    <span class="text-blue-400 ml-1">✓ Synced</span>
                    @else
                    <span class="text-slate-500 ml-1">Not Synced</span>
                    @endif
                </div>
                <div>
            ...
            <div class="flex flex-wrap gap-2 pt-3 border-t border-slate-700">
                <!-- View Button -->
                <a href="{{ route('admin.users.show', $user) }}"
                    class="inline-flex items-center gap-1 px-3 py-2 text-xs bg-lime-500/10 text-lime-400 rounded-lg hover:bg-lime-500/20 transition font-medium">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <!-- Edit Button -->
                <a href="{{ route('admin.users.edit', $user) }}"
                    class="inline-flex items-center gap-1 px-3 py-2 text-xs bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500/20 transition font-medium">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                ...
                <div class="relative flex-1 min-w-[120px]">
                    <button type="button" onclick="togglePasswordMenu('password-menu-mobile-{{ $user->id }}')"
                        class="w-full inline-flex items-center justify-center gap-1 px-3 py-2 text-xs bg-yellow-500/10 text-yellow-400 rounded-lg hover:bg-yellow-500/20 transition font-medium">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                        </svg>
                    ...
                    <div id="password-menu-mobile-{{ $user->id }}"
                        class="hidden absolute left-0 mt-2 w-72 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden">
                        <button type="button"
                            onclick="openSetPasswordModal({{ $user->id }}, '{{ $user->name }}', '{{ $user->email }}')"
                ...
                <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline-block">
                    ...
                    <button type="submit" onclick="return confirm('Are you sure you want to delete this user?')"
                        class="inline-flex items-center gap-1 px-3 py-2 text-xs bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition font-medium">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
...
<script>
    ...
    function togglePasswordMenu(menuId) {
        ...
        document.querySelectorAll('[id^="password-menu-"]').forEach(m => {
            if (m.id !== menuId) {
