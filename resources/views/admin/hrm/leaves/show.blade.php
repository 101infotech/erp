...
@section('content')
    <div class="space-y-6 max-w-4xl">
        ...
        <div class="bg-slate-800 rounded-lg p-6 border border-slate-700">
            ...
            <div class="flex gap-4">
                <!-- Approve Button -->
                <form method="POST" action="{{ route('admin.hrm.leaves.approve', $leave->id) }}" class="flex-1">
                    @csrf
                    <button type="submit"
                        onclick="return confirm('Are you sure you want to approve this leave request?')"
                        class="w-full px-6 py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition">
        ...
        <div class="bg-slate-800 rounded-lg p-6 border border-slate-700">
            <form method="POST" action="{{ route('admin.hrm.leaves.cancel', $leave->id) }}">
                @csrf
                <button type="submit"
                    onclick="return confirm('Are you sure you want to cancel this leave request? This will restore the leave balance if it was approved.')"
    ...
    <div id="rejectModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-slate-800 rounded-lg p-6 max-w-md w-full mx-4 border border-slate-700">
            <h3 class="text-xl font-semibold text-white mb-4">Reject Leave Request</h3>
