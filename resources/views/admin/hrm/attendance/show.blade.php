    ...
    <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg">
        <div
            ...
            <div>
                ...
                <p class="text-sm text-slate-400">{{ $attendance->employee->code }} • {{ $attendance->employee->email }}
                </p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-slate-700">
            <div class="px-6 py-4">
                ...
                <div class="text-xs text-slate-500">{{ \Carbon\Carbon::parse($attendance->date)->format('l') }}</div>
            </div>
            <div class="px-6 py-4">
                <div class="text-xs font-medium text-slate-400 uppercase tracking-wide">Tracked Hours</div>
                <div class="mt-2 text-lg font-semibold text-blue-400">{{ number_format($attendance->tracked_hours, 2)
                    }}h</div>
            </div>
            <div class="px-6 py-4">
                <div class="text-xs font-medium text-slate-400 uppercase tracking-wide">Payroll Hours</div>
                <div class="mt-2 text-lg font-semibold text-green-400">{{ number_format($attendance->payroll_hours, 2)
                    }}h</div>
            </div>
            <div class="px-6 py-4">
                <div class="text-xs font-medium text-slate-400 uppercase tracking-wide">Overtime</div>
                <div class="mt-2 text-lg font-semibold text-orange-400">{{ number_format($attendance->overtime_hours, 2)
                    }}h</div>
            </div>
        </div>
    </div>
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-lg font-bold text-white">{{ $entry->local_time->format('h:i A')
                                        }}</span>
                                    <span class="text-sm text-slate-500">{{ $entry->local_time->format('T') }}</span>
                                </div>
