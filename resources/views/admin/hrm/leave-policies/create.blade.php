...
<div class="px-6 md:px-8 py-6">
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center space-x-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-white">Create Leave Policy</h1>
        </div>
        <p class="text-slate-400 ml-9">Define leave policy for a company and leave type</p>
    </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.hrm.leave-policies.store') }}" method="POST"
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="policy_name" id="policy_name" value="{{ old('policy_name') }}" required
                    class="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-lime-500 focus:border-transparent"
            ...
            <div>
                ...
                <select name="leave_type" id="leave_type" required
                    ...
                    <option value="period" {{ old('leave_type')=='period' ? 'selected' : '' }}>Period Leave</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="gender_restriction" id="gender_restriction" required
                    class="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-lime-500 focus:border-transparent">
                    <option value="none" {{ old('gender_restriction')=='none' ? 'selected' : '' }}>No Restriction (All)
                    </option>
                    <option value="male" {{ old('gender_restriction')=='male' ? 'selected' : '' }}>Male Only</option>
                    <option value="female" {{ old('gender_restriction')=='female' ? 'selected' : '' }}>Female Only
                    </option>
                </select>
            ...
            <div>
                <label for="annual_quota" class="block text-sm font-medium text-slate-300 mb-2">Annual Quota (Days)
                    *</label>
                <input type="number" name="annual_quota" id="annual_quota" value="{{ old('annual_quota') }}" required
                    min="0" step="0.5"
                    class="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                    placeholder="e.g., 15">
            ...
            <div class="flex items-start space-x-3">
                <input type="checkbox" name="carry_forward_allowed" id="carry_forward_allowed" value="1" {{
                    old('carry_forward_allowed') ? 'checked' : '' }}
            ...
            <div id="carry_forward_section" class="hidden">
                ...
                <input type="number" name="max_carry_forward" id="max_carry_forward"
                    value="{{ old('max_carry_forward') }}" min="0" step="0.5"
                    class="w-full bg-slate-900 border border-slate-700 text-white rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-lime-500 focus:border-transparent"
                    placeholder="e.g., 5">
