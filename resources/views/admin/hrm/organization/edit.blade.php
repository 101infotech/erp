...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-slate-800 border border-slate-700 rounded-lg p-6">
        <form method="POST" action="{{ route('admin.hrm.organization.companies.update', $company) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="address" rows="3"
                    class="w-full px-4 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white focus:outline-none focus:ring-2 focus:ring-lime-500">{{ old('address', $company->address) }}</textarea>
            ...
            <div>
                ...
                <select name="finance_company_id" required
                    ...
                    <option value="{{ $f->id }}" {{ (int) old('finance_company_id', $company->finance_company_id) ===
                        (int)
                        $f->id ? 'selected' : '' }}>
                        {{ $f->name }}
                    </option>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit"
                    class="px-4 py-2 rounded-lg bg-lime-500 hover:bg-lime-600 text-slate-900 font-semibold">Update
                    Company</button>
            </div>
        </form>
    </div>
