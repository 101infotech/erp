...
@section('content')
<div class="px-6 md:px-8 space-y-6 py-6">
    <!-- Header -->
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Payroll Management</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Generate and manage employee payroll records</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
        <form method="GET" action="{{ route('admin.hrm.payroll.index') }}"
            ...
            <div class="flex-1 min-w-[180px]">
                ...
                <select name="status"
                    class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-lg px-2.5 py-1.5 focus:ring-2 focus:ring-lime-500 focus:border-transparent h-9">
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Employee</label>
            ...
            <!-- Company Filter -->
            <div class="flex-1 min-w-[200px]">
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Company</label>
            ...
            <!-- Period Filter -->
            <div class="flex-1 min-w-[180px]">
                <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Period From</label>
                <input type="date" name="period_start" value="{{ request('period_start') }}"
                    class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-lg px-2.5 py-1.5 focus:ring-2 focus:ring-lime-500 focus:border-transparent h-9">
            </div>

            <div class="flex-none min-w-[140px] flex items-end">
                <button type="submit"
                    class="w-full px-4 py-1.5 bg-lime-500 hover:bg-lime-600 text-slate-900 text-sm font-semibold rounded-lg transition whitespace-nowrap">
                    Apply Filters
                </button>
            </div>
        </form>
            ...
            <table class="w-full min-w-[1000px]">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
                    ...
                    <tr class="hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer">
                        ...
                        <td
                            class="px-4 py-3 text-right text-sm font-medium text-slate-900 dark:text-white whitespace-nowrap">
                            NPR {{ number_format($payroll->gross_salary, 2) }}
                        </td>
                        <td class="px-4 py-3 text-right text-sm font-medium text-lime-400 whitespace-nowrap">
                        ...
                        <td class="px-4 py-3 text-center text-sm whitespace-nowrap">
                            <a href="{{ route('admin.hrm.payroll.show', $payroll->id) }}"
                                class="text-lime-400 hover:text-lime-300 text-xs font-medium">View</a>
                            @if($payroll->status === 'draft')
                            ...
                            <button type="button"
                                ...
                                class="text-red-400 hover:text-red-300 text-xs font-medium">Delete</button>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-slate-400">
                            <svg class="mx-auto h-10 w-10 text-slate-600" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
...
<div id="deletePayrollModal"
    ...
    <div class="bg-slate-800 border border-slate-700 rounded-lg shadow-xl max-w-md w-full">
        <div class="flex items-center justify-between p-4 border-b border-slate-700">
            <h3 class="text-lg font-semibold text-white">Delete Draft Payroll</h3>
        ...
        <div class="p-4">
            <div class="flex items-start space-x-3 mb-4">
                ...
                <div class="flex-1">
                    ...
                    <div class="bg-slate-900/50 rounded p-2.5 mb-2.5 border border-slate-700">
                        <p class="text-xs text-white"><span class="font-medium">Employee:</span> <span
                                id="deleteEmployeeName"></span></p>
                        <p class="text-xs text-slate-400"><span class="font-medium">Period:</span> <span
            ...
            <form method="POST" action="" id="deletePayrollForm">
                ...
                <div class="flex justify-end gap-2.5">
                    <button type="button" onclick="closeModal('deletePayrollModal')"
                        class="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-white text-sm font-semibold rounded-lg transition">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-lg transition inline-flex items-center gap-2">
