...
<div id="salaryConfigModal"
    class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
    <div class="bg-slate-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
        <!-- Modal Header -->
        <div class="px-6 py-4 border-b border-slate-700 flex items-center justify-between flex-shrink-0">
            ...
            <button type="button" onclick="closeSalaryConfigModal()" class="text-slate-400 hover:text-white transition">
        ...
        <div class="p-6 overflow-y-auto flex-1 min-h-0">
            <div class="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 mb-6">
                <div class="flex items-start">
                    ...
                    <div class="ml-3 text-sm text-blue-300">
                        ...
                        <p class="text-blue-200/80">You can set individual salaries here or bulk update all at once.
            ...
            <div>
                ...
                <div id="employeeSalaryList" class="space-y-3">
                    ...
                    @if(!isset($employee->basic_salary_npr) || $employee->basic_salary_npr <= 0)
                        <div class="bg-slate-700/30 rounded-lg p-4 employee-salary-row"
                            data-employee-id="{{ $employee->id }}">
                            <div class="flex items-center justify-between mb-3">
                                <div>
                                    <p class="text-white font-medium">{{ $employee->name }}</p>
                                    <p class="text-slate-400 text-sm">{{ $employee->company->name ?? 'No Company' }}</p>
                                </div>
        ...
        <!-- Modal Footer -->
        <div class="px-6 py-4 border-t border-slate-700 flex items-center justify-between flex-shrink-0">
...
<script>
    ...
    function openSalaryConfigModal() {
        document.getElementById('salaryConfigModal').classList.remove('hidden');
        document.getElementById('salaryConfigModal').classList.add('flex');
    }
    
    function closeSalaryConfigModal() {
        document.getElementById('salaryConfigModal').classList.add('hidden');
        document.getElementById('salaryConfigModal').classList.remove('flex');
    }
