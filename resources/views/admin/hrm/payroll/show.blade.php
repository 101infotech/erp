...
<div class="px-6 md:px-8 py-6 space-y-6">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Payslip Details</h1>
            <p class="text-slate-400 text-sm mt-1">{{ $payroll->employee->full_name }} - {{
    ...
    <div class="flex flex-wrap gap-3 items-start mb-6">
        ...
        <button type="button" onclick="openModal('regeneratePayrollModal')"
            class="px-4 py-2.5 bg-purple-500 hover:bg-purple-600 text-white font-semibold rounded-lg transition inline-flex items-center gap-2 whitespace-nowrap"
            title="Recalculate payroll with latest data including any new holidays">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-4 pt-4 border-t border-slate-700">
                    ...
                    <div class="bg-blue-500/5 rounded-lg p-4 space-y-3">
                        ...
                        <div class="bg-slate-900/50 rounded px-3 py-2 space-y-1 text-sm">
                            ...
                            $weekendDays = $payroll->weekend_days ?? 0;
                            @endphp
                            ...
                            @if($holidayDays > 0)
                            <div class="flex justify-between">
                                <span class="text-slate-400">Company Holidays (Paid):</span>
                                <span class="text-pink-400">{{ $holidayDays }}</span>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="text-xs text-slate-500 italic px-2 py-1">
                            ...
                            (weekends)@if($holidayDays > 0) + {{ $holidayDays }} (holidays)@endif = {{
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <form method="POST" action="{{ route('admin.hrm.payroll.update', $payroll->id) }}">
                                    ...
                                    <button type="submit"
                                        class="px-3 py-1.5 text-sm bg-yellow-500 text-slate-900 font-semibold rounded-md hover:bg-yellow-400 transition">
                                ...
                                <form method="POST" action="{{ route('admin.hrm.payroll.update', $payroll->id) }}">
                                    ...
                                    <button type="submit"
                                        class="px-3 py-1.5 text-sm bg-slate-700 text-slate-100 font-semibold rounded-md hover:bg-slate-600 transition">
                                        Keep deduction at 0
                                    </button>
                                </form>
                            </div>
...
<x-professional-modal id="regeneratePayrollModal" title="Regenerate Payroll"
    subtitle="Recalculate payroll with latest data" icon="warning" iconColor="purple" maxWidth="max-w-xl">
    <div class="space-y-4">
        <div class="bg-purple-500/10 border border-purple-500/30 rounded-lg p-4">
            <div class="flex items-start gap-3">
                ...
                <div class="text-sm text-purple-200">
                    <p class="font-medium mb-2">What happens when you regenerate:</p>
                    <ul class="list-disc ml-4 space-y-1">
                        <li>Latest attendance data will be fetched</li>
                        <li>New company holidays will be accounted for</li>
                        <li>Hours and deductions will be recalculated</li>
                        <li>Manual edits (OT payment, hourly deduction) will be preserved</li>
                        <li>Anomaly review status will be reset</li>
                        <li>PDF will be regenerated</li>
                    </ul>
                </div>
            </div>
        </div>
        <p class="text-slate-300">Are you sure you want to regenerate this payroll?</p>
        <div class="bg-slate-900/50 rounded-lg p-3 border border-slate-700">
            <p class="text-sm text-white"><span class="font-medium">Employee:</span> {{
                $payroll->employee->full_name }}</p>
            <p class="text-sm text-slate-400 mt-1"><span class="font-medium">Period:</span> {{
                $payroll->period_start_bs }} to {{ $payroll->period_end_bs }}</p>
        </div>
    </div>
    <x-slot name="footer">
        <button onclick="closeModal('regeneratePayrollModal')"
            class="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-white font-semibold rounded-lg transition">Cancel</button>
        <form method="POST" action="{{ route('admin.hrm.payroll.regenerate', $payroll->id) }}" class="inline">
            @csrf
            <button type="submit"
                class="px-4 py-2.5 bg-purple-500 hover:bg-purple-600 text-white font-semibold rounded-lg transition inline-flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
