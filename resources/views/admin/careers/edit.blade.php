...
<div class="max-w-3xl">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('admin.careers.index') }}" class="group">
        ...
        <form action="{{ route('admin.careers.update', $career) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="site_id" id="site_id" required
                    ...
                    <option value="{{ $site->id }}" {{ old('site_id', $career->site_id) == $site->id ? 'selected' : ''
                        }}>
                        {{ $site->name }}
                    </option>
                    @endforeach
            ...
            <div class="mb-4">
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $career->slug) }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">{{ old('description', $career->description) }}</textarea>
            </div>

            <div class="mb-4">
                <label for="requirements" class="block text-sm font-medium text-gray-700 mb-2">Requirements</label>
                <textarea name="requirements" id="requirements" rows="4"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">{{ old('requirements', $career->requirements) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="responsibilities" id="responsibilities" rows="4"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">{{ old('responsibilities', $career->responsibilities) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="job_type" id="job_type" required
                    ...
                    <option value="full-time" {{ old('job_type', $career->job_type) == 'full-time' ? 'selected' : ''
                        }}>Full-time</option>
                    <option value="part-time" {{ old('job_type', $career->job_type) == 'part-time' ? 'selected' : ''
                        }}>Part-time</option>
                    <option value="contract" {{ old('job_type', $career->job_type) == 'contract' ? 'selected' : ''
                        }}>Contract</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $career->is_active) ?
                    'checked' : '' }}
