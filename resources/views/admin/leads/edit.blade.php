...
@section('page-title', 'Edit Service Lead')
...
<div class="max-w-4xl">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-2 text-sm text-slate-400 mb-2">
            ...
            <span>/</span>
            <a href="{{ route('admin.leads.show', $lead) }}" class="hover:text-lime-400 transition">{{
                $lead->client_name }}</a>
            <span>/</span>
            <span class="text-white">Edit</span>
        </div>
        <h2 class="text-2xl font-bold text-white">Edit Service Lead</h2>
        <p class="text-slate-400 mt-1">Update service request information</p>
    </div>
    ...
    <form action="{{ route('admin.leads.update', $lead) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" required value="{{ old('email', $lead->email) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="service_requested" name="service_requested" required
                        ...
                        @php
                        $services = [
                        'Home Inspection', 'Commercial Inspection', 'New Construction', 'Renovation',
                        'Basement Development', 'Kitchen Remodel', 'Bathroom Remodel', 'Deck/Patio',
                        'Flooring', 'Painting', 'Roofing', 'Siding', 'Windows/Doors', 'Plumbing', 'Electrical'
                        ];
                        @endphp
                        @foreach($services as $service)
                        <option value="{{ $service }}" {{ old('service_requested', $lead->service_requested) == $service
                            ? 'selected' : '' }}>
                            {{ $service }}
                        </option>
                        @endforeach
                ...
                <div>
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="{{ $status->status_key }}" {{ old('status', $lead->status) == $status->status_key
                            ? 'selected' : '' }}>
                            {{ $status->display_name }}
                        </option>
                        @endforeach
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="message" name="message" rows="3"
                        class="w-full px-3 py-2 bg-slate-700 border border-slate-600 text-white placeholder-slate-400 rounded-lg focus:ring-2 focus:ring-lime-500 focus:border-lime-500 @error('message') border-red-500 @enderror">{{ old('message', $lead->message) }}</textarea>
                    @error('message')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" id="inspection_date" name="inspection_date"
                        value="{{ old('inspection_date', $lead->inspection_date?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="text" id="inspection_time" name="inspection_time"
                        value="{{ old('inspection_time', $lead->inspection_time?->format('H:i')) }}"
                ...
                <div>
                    ...
                    <input type="number" id="inspection_charge" name="inspection_charge"
                        value="{{ old('inspection_charge', $lead->inspection_charge) }}" step="0.01" min="0"
                        placeholder="0.00"
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="project_size" name="project_size"
                        value="{{ old('project_size', $lead->project_size) }}" placeholder="e.g., 2000 sq ft, 3 bedroom"
                ...
                <div>
                    ...
                    <select id="assigned_to" name="assigned_to"
                        ...
                        <option value="{{ $user->id }}" {{ old('assigned_to', $lead->inspection_assigned_to) ==
                            $user->id ?
                            'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="timeline" name="timeline" value="{{ old('timeline', $lead->timeline) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="budget" name="budget" value="{{ old('budget', $lead->budget) }}"
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-4">
            <div class="flex items-center justify-between text-sm text-slate-400">
                <div>
                    Created by {{ $lead->createdBy->name ?? 'System' }} on {{ $lead->created_at->format('M d, Y h:i A')
                    }}
                </div>
                <div>
