...
<div class="max-w-7xl">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('admin.finance.purchases.index') }}" class="group">
            <svg class="w-6 h-6 text-slate-400 group-hover:text-white transition-colors" fill="none"
                stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
            </svg>
        </a>
        <h2 class="text-2xl font-bold text-white">Edit Purchase Bill</h2>
    </div>
        ...
        <form action="{{ route('admin.finance.purchases.update', $purchase) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="vendor_name" value="{{ old('vendor_name', $purchase->vendor_name) }}"
                ...
                <div>
                    ...
                    <select name="payment_method"
                        ...
                        <option value="bank_transfer" {{ old('payment_method', $purchase->payment_method) ==
                            'bank_transfer'
                            ? 'selected' : '' }}>Bank Transfer</option>
                        <option value="cheque" {{ old('payment_method', $purchase->payment_method) == 'cheque' ?
                            'selected'
                            : '' }}>Cheque</option>
                        <option value="card" {{ old('payment_method', $purchase->payment_method) == 'card' ? 'selected'
                            : ''
                            }}>Card</option>
                        <option value="online" {{ old('payment_method', $purchase->payment_method) == 'online' ?
                            'selected'
                            : '' }}>Online</option>
                    </select>
