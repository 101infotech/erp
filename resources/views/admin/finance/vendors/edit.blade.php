...
<div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6 max-w-4xl mx-auto">
    ...
    <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-6">Edit Vendor</h2>

    <form action="{{ route('admin.finance.vendors.update', $vendor) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" value="{{ $vendor->company->name }}" readonly
            ...
            <div>
                ...
                <select name="vendor_type" required
                    class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white">
                    <option value="supplier" {{ old('vendor_type', $vendor->vendor_type)=='supplier' ? 'selected' : ''
                        }}>Supplier</option>
                    <option value="contractor" {{ old('vendor_type', $vendor->vendor_type)=='contractor' ? 'selected' :
                        '' }}>Contractor</option>
                    <option value="service_provider" {{ old('vendor_type', $vendor->vendor_type)=='service_provider' ?
                        'selected' : '' }}>Service Provider</option>
                </select>
                @error('vendor_type')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="pan_number" value="{{ old('pan_number', $vendor->pan_number) }}"
                    class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white">
                @error('pan_number')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="address" rows="2"
                    class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white">{{ old('address', $vendor->address) }}</textarea>
                @error('address')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
