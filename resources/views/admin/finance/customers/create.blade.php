...
<div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6 max-w-4xl mx-auto">
    ...
    <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-6">Create New Customer</h2>

    <form action="{{ route('admin.finance.customers.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="customer_code" value="{{ old('customer_code') }}" placeholder="Auto-generated"
                    class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white">
                @error('customer_code')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="customer_type" required
                    class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white">
                    <option value="individual" {{ old('customer_type')=='individual' ? 'selected' : '' }}>Individual
                    </option>
                    <option value="corporate" {{ old('customer_type')=='corporate' ? 'selected' : '' }}>Corporate
                    </option>
                    <option value="government" {{ old('customer_type')=='government' ? 'selected' : '' }}>Government
                    </option>
                </select>
                @error('customer_type')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
