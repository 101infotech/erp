...
<div class="max-w-7xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-2xl font-bold text-white">Edit Account</h2>
    </div>

    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
        <form action="{{ route('admin.finance.accounts.update', $account) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="company_id" required
                        ...
                        <option value="{{ $company->id }}" {{ old('company_id', $account->company_id) == $company->id ?
                            'selected' : '' }}>
                            {{ $company->name }}
                        </option>
                        @endforeach
                ...
                <div>
                    ...
                    <select name="account_type" required
                        class="w-full px-4 py-2 border rounded-lg dark:bg-slate-700 dark:text-white">
                        <option value="asset" {{ old('account_type', $account->account_type) == 'asset' ? 'selected' :
                            ''
                            }}>Asset</option>
                        <option value="liability" {{ old('account_type', $account->account_type) == 'liability' ?
                            'selected'
                            : '' }}>Liability</option>
                        <option value="equity" {{ old('account_type', $account->account_type) == 'equity' ? 'selected' :
                            ''
                            }}>Equity</option>
                        <option value="revenue" {{ old('account_type', $account->account_type) == 'revenue' ? 'selected'
                            :
                            '' }}>Revenue</option>
                        <option value="expense" {{ old('account_type', $account->account_type) == 'expense' ? 'selected'
                            :
                            '' }}>Expense</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="parent_account_id"
                        ...
                        <option value="{{ $parent->id }}" {{ old('parent_account_id', $account->parent_account_id) ==
                            $parent->id ? 'selected' : '' }}>
                ...
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $account->is_active) ?
                        'checked' : '' }}
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-2 border rounded-lg dark:bg-slate-700 dark:text-white">{{ old('description', $account->description) }}</textarea>
