...
@section('page-title', 'Edit Founder')
...
<div class="max-w-7xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h2 class="text-2xl font-bold text-white">Edit Founder: {{ $founder->name }}</h2>
    </div>

    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Founder: {{ $founder->name }}</h2>
            <p class="text-slate-600 dark:text-slate-400 mt-1">Update founder information</p>
        </div>

        <form action="{{ route('admin.finance.founders.update', $founder) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="company_id" required
                        ...
                        <option value="{{ $company->id }}" {{ old('company_id', $founder->company_id) == $company->id ?
                            'selected' : '' }}>
                            {{ $company->name }}
                        </option>
                        @endforeach
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $founder->email) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone', $founder->phone) }}"
                ...
                <div>
                    ...
                    <input type="text" name="pan_number" value="{{ old('pan_number', $founder->pan_number) }}"
                ...
                <div>
                    ...
                    <input type="number" name="investment_limit"
                        value="{{ old('investment_limit', $founder->investment_limit) }}" step="0.01" min="0" required
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" rows="3"
                    class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 @error('notes') border-red-500 @enderror">{{ old('notes', $founder->notes) }}</textarea>
                @error('notes')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $founder->is_active) ?
                    'checked' :
                    '' }}
