...
<div class="bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 border border-slate-700 hover:border-{{ $color }}-500/50 transition-colors">
    <div class="flex items-center justify-between">
        <div>
            ...
            @endif
            @if(isset($metric))
            <p class="text-xs {{ str_contains($metric, '+') ? 'text-green-400' : 'text-red-400' }} mt-1">{{ $metric }}</p>
            @endif
        </div>
