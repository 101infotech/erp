...
<body class="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 min-h-screen flex items-center justify-center">
    <div class="max-w-2xl mx-auto px-4 text-center">
        ...
        <div class="mb-8">
            <div class="relative">
                <!-- Large 500 Text -->
                <h1
                    class="text-[180px] md:text-[240px] font-black text-transparent bg-clip-text bg-gradient-to-br from-orange-400 to-orange-600 leading-none select-none opacity-20">
                    500
                </h1>

                <!-- Icon Overlay -->
                <div class="absolute inset-0 flex items-center justify-center">
                    <svg class="w-32 h-32 md:w-40 md:h-40 text-orange-500" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
        ...
        <div class="mt-12 text-left">
            <details class="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                ...
                <div class="mt-4 space-y-2">
                    <p class="text-sm text-slate-300"><span class="font-semibold text-white">Message:</span> {{
                        $exception->getMessage() }}</p>
                    <p class="text-sm text-slate-300"><span class="font-semibold text-white">File:</span> {{
                        $exception->getFile() }}</p>
                    <p class="text-sm text-slate-300"><span class="font-semibold text-white">Line:</span> {{
                        $exception->getLine() }}</p>
                </div>
            </details>
        </div>
    ...
    <div class="fixed inset-0 pointer-events-none overflow-hidden -z-10">
        ...
        <div class="absolute bottom-20 right-10 w-96 h-96 bg-orange-600/5 rounded-full blur-3xl animate-pulse"
